...
def validate_tool_calls(
    ...
    requested_output_cap: Optional[int], truncated_tool_call_retries: int,
    ...
    if invalid_json_args:
        ...
        if _truncated:
            retries = truncated_tool_call_retries
            if retries < 4:
                retries += 1
                truncated_tool_call_retries = retries
                base = agent.max_tokens if agent.max_tokens else 4096
                requested = requested_output_cap or 0
                boost = max(base * (2 ** retries), requested or 0)
                agent._ephemeral_max_output_tokens = min(boost, max(32768, requested or 0))
                agent._buffer_vprint(
                    f"⚠️  Truncated tool call arguments detected — retrying API call ({retries}/4)..."
                )
                return _verdict("continue")
            agent._vprint(
