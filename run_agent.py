...
class AIAgent(
    ...
    def close(self) -> None:
        ...
        blocks the rest."""
        # Delegated children may still be unwinding their turn while eviction runs. Defer
        # teardown until the worker completes, then serialize competing close callers.
        worker = getattr(self, "_delegate_worker_future", None)
        if worker is not None and not worker.done():
            try:
                lock = getattr(self, "_delegate_close_lock", None)
                if lock is None:
                    lock = threading.Lock()
                    self._delegate_close_lock = lock
                with lock:
                    if not getattr(self, "_delegate_close_scheduled", False):
                        self._delegate_close_scheduled = True
                        worker.add_done_callback(lambda _future: self.close())
            except Exception:
                logger.error("Unable to defer delegated child close; keeping resources open until worker exit", exc_info=True)
            return
        lock = getattr(self, "_delegate_close_lock", None)
        if lock is not None:
            try:
                ...
                logger.debug("Unable to claim delegated child close lock", exc_info=True)
