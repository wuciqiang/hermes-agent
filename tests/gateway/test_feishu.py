...
class TestAdapterBehavior(unittest.TestCase):
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_send_replies_in_thread_for_top_level_message_by_default(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_send_can_disable_feishu_threaded_replies(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_send_uses_metadata_reply_target_without_thread_id(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
        ...
        with patch("gateway.platforms.feishu.asyncio.to_thread", side_effect=_direct):
            result = asyncio.run(
                adapter.send(
                    chat_id="oc_chat",
                    content="status update",
                    metadata={"reply_to_message_id": "om_trigger"},
                )
            )
