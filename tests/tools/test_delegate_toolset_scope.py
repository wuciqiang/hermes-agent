...
class TestToolsetIntersection:
    ...
    def test_strip_blocked_removes_delegation(self):
        ...
        child = _strip_blocked_tools(
            ["terminal", "delegation", "clarify", "memory", "backlinkhub"]
        )
        assert "delegation" not in child
        assert "clarify" not in child
        assert "memory" not in child
        assert "backlinkhub" not in child
        assert "terminal" in child

    def test_backlink_worker_may_use_only_explicit_backlinkhub_scope(self):
        child = _strip_blocked_tools(
            ["terminal", "backlinkhub", "delegation"], allow_backlinkhub=True
        )
        assert child == ["terminal", "backlinkhub"]
