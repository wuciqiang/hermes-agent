...
_ROUTING_ORIGIN_FIELDS = (
    "platform", "chat_id", "chat_type", "thread_id", "message_id",
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            key: value
            for key in _ROUTING_ORIGIN_FIELDS
            if (value := get_session_env(f"HERMES_SESSION_{key.upper()}", ""))
        }
    ...
    with _records_lock:
        ...
        if platform and chat_id:
            records.extend(
                ...
                and str(r.get("platform") or "") == platform
                and str(r.get("chat_id") or "") == chat_id
                and (not user_id or str(r.get("user_id") or "") == user_id)
...
def _push_completion_event(record: Dict[str, Any], result: Dict[str, Any], status: str) -> None:
    ...
    evt = {
        ...
        **{k: record[k] for k in _ROUTING_ORIGIN_FIELDS if record.get(k)},
...
def interrupt_for_session(
    ...
    platform: str = "", chat_id: str = "", user_id: str = "", reason: str = "session_end",
) -> int:
