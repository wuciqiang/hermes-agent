...
def _filter_secret_env(
    ...
    for key, value in items.items():
        ...
        if (_is_hermes_internal_secret(key) and not first_party) or key in plugin_strip:
            continue
...
def _scrubbed_env(parts, plugin_strip: frozenset, fix_path) -> dict:
    ...
    *fix_path* (always prepending the hermes install dir so bare ``hermes`` resolves
    ...
    source_env = {}
    for items, _unwrap_force in parts:
        source_env.update(items)
    try:
        from tools.env_passthrough import resolve_all_passthrough_values
        out.update(resolve_all_passthrough_values(source_env))
    except Exception:
        pass
