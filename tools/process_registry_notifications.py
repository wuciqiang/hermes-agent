...
def _format_batch_delegation(evt: dict, deleg_id: str, completed_at: float) -> str:
    ...
    stopped = str(evt.get("interrupt_reason") or "") in {"user_stop", "session_reset"}
    summary_intro = (
        f"A background fan-out unit {unit} was explicitly stopped by the user. "
        "Report partial results only; Do not dispatch replacement work automatically."
        if stopped else
    ...
    cumulative_tokens = evt.get("cumulative_tokens") or {}
    usage_parts = []
    if evt.get("continuation_segments") is not None:
        usage_parts.append(f"segments={evt['continuation_segments']}")
    if evt.get("cumulative_api_calls") is not None:
        usage_parts.append(f"api_calls={evt['cumulative_api_calls']}")
    if cumulative_tokens.get("input") is not None:
        usage_parts.append(f"input_tokens={cumulative_tokens['input']}")
    if cumulative_tokens.get("output") is not None:
        usage_parts.append(f"output_tokens={cumulative_tokens['output']}")
    if evt.get("cumulative_reasoning_tokens") is not None:
        usage_parts.append(f"reasoning_tokens={evt['cumulative_reasoning_tokens']}")
    if evt.get("cumulative_duration_seconds") is not None:
        usage_parts.append(f"duration_seconds={evt['cumulative_duration_seconds']}")
    if evt.get("cumulative_cost_usd") is not None:
...
def _format_async_delegation(evt: dict) -> str:
    ...
    stopped = str(evt.get("interrupt_reason") or "") in {"user_stop", "session_reset"}
    intro = (
        "This background subagent was explicitly stopped by the user. Report partial results only. "
        "Do not dispatch replacement work automatically."
        if stopped else
